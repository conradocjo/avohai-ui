...
function addDeclarationToNgModule(options) {
    return (host) => {
        ...
        const componentPath = `/${options.path}/` +
            ...
            '.component';
        const relativePath = angular_1.buildRelativePath(modulePath, componentPath);
        const classifiedName = core_1.strings.classify(`${options.name}Component`);
        const declarationChanges = angular_1.addDeclarationToModule(source, modulePath, classifiedName, relativePath);
        ...
        if (options.export) {
            ...
            const exportChanges = angular_1.addExportToModule(source, modulePath, core_1.strings.classify(`${options.name}Component`), relativePath);
        ...
        if (options.entryComponent) {
            ...
            const entryComponentChanges = angular_1.addEntryComponentToModule(source, modulePath, core_1.strings.classify(`${options.name}Component`), relativePath);
...
function buildComponent(options, additionalFiles = {}) {
    return (host, context) => {
        const workspace = angular_1.getWorkspace(host);
        ...
        if (options.path === undefined) {
            ...
            options.path = angular_1.buildDefaultPath(project);
        }
        options.module = angular_1.findModuleFromOptions(host, options);
        const parsedPath = angular_1.parseName(options.path, options.name);
        options.name = parsedPath.name;
        options.path = parsedPath.path;
        options.selector = options.selector || buildSelector(options, project.prefix);
        angular_1.validateName(options.name);
        angular_1.validateHtmlSelector(options.selector);
        ...
        return schematics_1.chain([schematics_1.branchAndMerge(schematics_1.chain([addDeclarationToNgModule(options), schematics_1.mergeWith(templateSource)]))])(host, context);
    };
}
